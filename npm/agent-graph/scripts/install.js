...
const releaseBaseURL = "https://github.com/arcmantle/agent-graph/releases/download";
...
function platformAsset(platform = process.platform, architecture = process.arch) {
  ...
  return `agent-graph-${platform}-${architecture}.gz`;
}

function download(url, redirects = 0) {
  return new Promise((resolve, reject) => {
    const request = https.get(url, { headers: { "user-agent": "agent-graph-npm-installer" } }, response => {
...
if (require.main === module) {
  install().catch(error => {
    console.error(`agent-graph installation failed: ${error.message}`);
    process.exitCode = 1;
  });
}
