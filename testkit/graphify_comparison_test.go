...
func TestRunGraphifyComparisonCorpusComparesCLIAndExportedFacts(t *testing.T) {
	...
	command := exec.Command("go", "run", "../cmd/agent-graph", "index", "--database", database, "--format", "json", workspace.Root)
	if output, err := command.CombinedOutput(); err != nil {
		t.Fatalf("index comparison corpus workspace: %v\n%s", err, output)
	}

	command = exec.Command("go", "run", "../cmd/agent-graph", "export", "--database", database, "--format", "json", workspace.Root)
