...
func TestAcquireReusesLiveWorkspaceOwner(t *testing.T) {
	...
	if owner.Endpoint() != filepath.Join(root, ".agent-graph", "indexer.sock") {
...
func TestAcquireRecoversStaleOwnerMetadata(t *testing.T) {
	root := t.TempDir()
	state := filepath.Join(root, ".agent-graph")
...
func TestManagerStartsWorkspaceIndexerAndReportsStatus(t *testing.T) {
	...
	if status.Endpoint != filepath.Join(root, ".agent-graph", "indexer.sock") {
